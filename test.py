...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--data', type=str, default='/data2/zhouhan/YOLOv6-main/data/dataset_all_center_negative.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--task', default='test', help='test, val, or speed')
    ...
    parser.add_argument('--save_dir', type=str, default='/data2/zhouhan/YOLOv6-positive_negative/runs/train/positive_negative/eval/', help='evaluation save dir')
...
def run(data,
    ...
    # eval
    model.eval()
    pred_result, cancer, ulcer, negative = val.predict_model(model, dataloader, task)
    return cancer, ulcer, negative


def main(args):
    cancer, ulcer, negative = run(**vars(args))
    return cancer, ulcer, negative

if __name__ == "__main__":

    weights_path = "/data2/zhouhan/YOLOv6-positive_negative/runs/train/positive_negative/weights"
    ...
    for i in weight_list:
        ...
        args.weights = os.path.join(weights_path, i)
        cancer, ulcer, negative = main(args)
        json_dict[i]={}
        json_dict[i]["cancer"] = cancer*100
        json_dict[i]["ulcer"] = ulcer*100
        json_dict[i]["negative"] = negative*100

        print(i)

        pred_json_path = os.path.join(weights_path, "result.json")
